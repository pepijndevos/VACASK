...
std::vector<OsdiFile*>OsdiFile::fileOrder;
std::unordered_map<std::string,OsdiFile*> OsdiFile::nameToFile;
...
OsdiFile::OsdiFile(void* handle_, std::string file_, Status& s) 
    ...
    // Attributes table
    attributes = ((OsdiAttribute*)dynamicLibrarySymbol(handle, "OSDI_ATTRIBUTES"));
...
OsdiFile* OsdiFile::open(std::string file, const Loc& location, Status& s) {
    ...
    if (inserted) {
        // Inserted successfully, add to name map and order
        auto osdiFile = it->second.get();
        fileOrder.push_back(osdiFile);
        nameToFile.insert(std::make_pair(file, osdiFile));
        return osdiFile;
    } else {
        return nullptr;
    }
...
void OsdiFile::dump(int indent, std::ostream& os) const {
    // Attribute formatter
    auto attribute = [this](OsdiAttribute* a) -> std::string {
        std::stringstream ss;
        ss << std::defaultfloat;
        switch (a->value_type) {
            case ATTR_TYPE_INT:
                return std::to_string(a->value.integer);
            case ATTR_TYPE_REAL:
                ss.str(""); ss << a->value.real;
                return ss.str(); 
            case ATTR_TYPE_STR:
                return std::string("\"")+a->value.string+"\""; 
        }
        return "";
    };
    ...
    if (naturesCount>0) {
        os << pfx << "Natures:\n";
        for(decltype(naturesCount) i=0; i<naturesCount; i++) {
            auto nat = natures+i;
            os << pfx << "  " << i << ": " << nat->name << "\n";
            switch (nat->parent_type) {
                case NATREF_NATURE:
                    os << pfx << "    parent: nature("  << nat->parent << ")\n"; break;
                case NATREF_DISCIPLINE_FLOW:
                    os << pfx << "    parent: discipline("  << nat->parent << ") flow\n"; break;
                case NATREF_DISCIPLINE_POTENTIAL:
                    os << pfx << "    parent: discipline("  << nat->parent << ") potential\n"; break;
            }
            if (nat->ddt!=UINT32_MAX) {
                os << pfx << "    ddt nature: nature("  << nat->ddt << ")\n";
            }
            if (nat->idt!=UINT32_MAX) {
                os << pfx << "    idt nature: nature("  << nat->idt << ")\n";
            }
            os << pfx << "    attributes ("  << nat->num_attr << ")\n";
            if (attributes) {
                for(decltype(nat->num_attr) j=0; j<nat->num_attr; j++) {
                    auto attr = attributes+nat->attr_start+j;
                    os << pfx << "      " << attr->name << " = " << attribute(attr) << "\n";
                }
            }
        }
    }
    ...
    if (disciplinesCount) {
        os << pfx << "Disciplines:\n";
        for(decltype(disciplinesCount) i=0; i<disciplinesCount; i++) {
            auto disc = disciplines+i;
            os << pfx << "  " << i << ": " << disc->name << "\n";
            if (disc->flow!=UINT32_MAX) {
                os << pfx << "    flow: nature("  << disc->flow << ")\n";
            }
            if (disc->potential!=UINT32_MAX) {
                os << pfx << "    potential: nature("  << disc->potential << ")\n";
            }
            if (disc->domain!=DOMAIN_NOT_GIVEN) {
                os << pfx << "    domain: "  << (disc->domain==DOMAIN_CONTINUOUS ? "contiuous" : "discrete" ) << "\n";
            }
            if (attributes) {
                auto at = disc->attr_start;
                if (disc->num_flow_attr) {
                    os << pfx << "    flow attributes ("  << disc->num_flow_attr << ")\n";
                    for(decltype(disc->num_flow_attr) j=0; j<disc->num_flow_attr; j++) {
                        auto attr = attributes+at+j;
                        os << pfx << "      " << attr->name << " = " << attribute(attr) << "\n";
                    }
                }
                at += disc->num_flow_attr;
                if (disc->num_potential_attr) {
                    os << pfx << "    potential attributes ("  << disc->num_potential_attr << ")\n";
                    for(decltype(disc->num_potential_attr) j=0; j<disc->num_potential_attr; j++) {
                        auto attr = attributes+at+j;
                        os << pfx << "      " << attr->name << " = " << attribute(attr) << "\n";
                    }
                }
                ...
                if (disc->num_user_attr) {
                    os << pfx << "    user attributes ("  << disc->num_user_attr << ")\n";
                    for(decltype(disc->num_user_attr) j=0; j<disc->num_user_attr; j++) {
                        auto attr = attributes+at+j;
                        os << pfx << "      " << attr->name << " = " << attribute(attr) << "\n";
                    }
                }   
    ...
    if (descriptorCount>0) {
        os << pfx << "Modules:\n";
        for(decltype(descriptorCount) i=0; i<descriptorCount; i++) {
            os << pfx << "  " << i << ": \"" << descriptors[i]->name << "\" exposed as \"" << namesArray[i] << "\"\n";
        }
    }
}
