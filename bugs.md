...
- [ ] **1. Bisection terminates after a single step (wrong PWL value on large index jumps).**
...
- [ ] **2. Periodic breakpoint period-offset (`i1Per`) is never updated — dead store + off-by-one-period break times.**
...
- [ ] **3. `pwlIndexLookup` is not self-guarding against `target == t[n-1]`.**
...
- [ ] **4. `DBGCHECK(dx<=0, ...)` is a release no-op guarding a division.**
...
- [ ] **5. `separate()` size_t underflow → out-of-bounds read when offset >= vector length.**
...
- [ ] **6. Duplicate `return true;` (dead code).**
...
- [ ] **7. `interleave` length variable is `int`, compared against `size_t`.**
...
- [ ] **10. `timeTol` computed twice / first copy unused.**
...
- [ ] **11. `using ssize_t` shadows the POSIX global and is declared after a use.**
...
- [ ] **12. `FwMinAggregate` dereferences `x[0]` / `begin()+1` without an empty-vector guard.**
  [include/rpnfunctor.h:448](include/rpnfunctor.h#L448) — the corrected loop now seeds
  `res = x[0]` and starts at `begin()+1`. Correct for non-empty input, but UB if a zero-length
  vector ever reaches it. Confirm callers guarantee non-empty (the `max` aggregate likely has the
  same shape — check it too).
