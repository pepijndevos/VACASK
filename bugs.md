...
- [ ] **8.** **[lib/filestack.cpp:170-178](lib/filestack.cpp#L170-L178), [224-229](lib/filestack.cpp#L224-L229)** — Off-by-one in `addRpnString`:
  ```cpp
  rpnStringStack.push_back(...);
  return -(rpnStringStack.size()+1);   // post-push size is N, returns -(N+1)
  ```
  `cString` then does `rpnStringStack[-id-1]` and indexes element `N` (out of bounds). Should be `return -rpnStringStack.size();` (or compute the id before the `push_back`).

- [ ] **9.** **[lib/parser.cpp:54-67](lib/parser.cpp#L54-L67), [122-147](lib/parser.cpp#L122-L147), [188-211](lib/parser.cpp#L188-L211)** — The `const std::string&&` rvalue-overloads do `stream.str(std::move(input))`. `std::move` on a `const` r-value yields a `const` r-value, which falls back to copy; the function is no faster than the lvalue overload but pretends to move (per its own comment). Also `parseNetlistString(const std::string&&, ...)` ([line 54](lib/parser.cpp#L54)) is missing the `acctNew.parse++` increment that the lvalue overload has at [line 40](lib/parser.cpp#L40), so the parse counter under-counts when callers pass r-value strings.

- [ ] **10.** **[simulator/main.cpp:29](simulator/main.cpp#L29) vs [109](simulator/main.cpp#L109)** — Help text advertises `--extratomlfile` (no hyphen) but the parser only accepts `--extra-tomlfile`. Either the help is wrong or the option name is wrong; users cannot trigger the documented switch.
