...
- [ ] **1. Uninitialized `solver_` in `NRSolver`** — `include/nrsolver.h:249`
...
- [ ] **2. Uninitialized `cxSolver_` in the AC-family cores** — `include/coreac.h:163`
...
- [ ] **3. Stale duplicate header `include/cirdelay.h`** — new file this branch, 250 lines
...
- [ ] **4. Unused local `options` in `ACCore::rebuild()`** — `lib/coreac.cpp:154`
...
- [ ] **6. `Simulator::setup()` no longer idempotent** — `lib/simulator.cpp:96`
  ...
  reason. Only bites embedders/tests that call `setup()` twice — confirm whether
  that is a supported path.

- [ ] **7. Dead protected methods in `solklu.h`** — `structuralRank()`,
